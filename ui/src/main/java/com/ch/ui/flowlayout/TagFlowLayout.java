...
public class TagFlowLayout extends FlowLayout
    ...
    private boolean isSingle = false;//-1为不限制数量
    ...
    public TagFlowLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        isSingle = ta.getBoolean(R.styleable.TagFlowLayout_isSingle, false);
        ta.recycle();
    }
    ...
    private void changeAdapter() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if (preCheckedList.contains(i)||mSelectedView.contains(i)) {
                setChildChecked(i, tagViewContainer);
            }else {
                setChildUnChecked(i,tagViewContainer);
            }
    ...
    private void doSelect(TagView child, int position) {
        if (!child.isChecked()) {
            ...
            //当为单选，且选择数量为1时
            if (mSelectedMax > 0 && mSelectedView.size() == 1 && isSingle) {
                return;
            } else {
    ...
    public void setUnSelect(int position){
        if (mSelectedView.contains(position)){
            mSelectedView.remove(position);
            changeAdapter();
        }
    }
